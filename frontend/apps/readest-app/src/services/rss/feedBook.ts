...
export function generateFeedCoverSvg(_feedUrl: string, _title: string): EpubImage {
  const iconBytes = new TextEncoder().encode(RSS_ICON_SVG);
  return { format: 'svg', data: iconBytes };
}

export function createFeedBook(_feed: ParsedFeed, _image?: EpubImage): Partial<Book> {
  return {};
}

export function rasterizeCoverSvg(_svg: string, _size: number): Promise<Blob | null> {
