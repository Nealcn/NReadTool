...
// Always ensures ai_explain is present (for both new and existing configs).
const sanitize = (items: AnnotationToolType[] | undefined): AnnotationToolType[] => {
  ...
  // Ensure ai_explain is always in the toolbar (insert before share, or append)
  if (!seen.has('ai_explain')) {
    const shareIdx = out.indexOf('share');
    if (shareIdx >= 0) {
      out.splice(shareIdx, 0, 'ai_explain');
    } else {
      out.push('ai_explain');
    }
    seen.add('ai_explain');
